...
public class Main {
    ...
    public List<String> q5() {
        return players
                .stream()
                .sorted(
                        Comparator.comparingInt(Player::getAge).reversed()
                                .thenComparing(Comparator.comparingDouble(Player::getSalary).reversed())
                )
                .map(Player::getFullName)
                .limit(10)
                .collect(Collectors.toList());
    ...
    public Map<Integer, Integer> q6() {
        Map<Integer, Long> result = players
                .stream()
                .map(Player::getAge)
        ...
        return result.entrySet()
                .stream()
                .collect(Collectors.toMap(Map.Entry::getKey, e -> (int)(long)e.getValue()));
    }
